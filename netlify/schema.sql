...
-- fewer writes than the previous row-per-request design.
...
-- Legacy row-per-request table from the v1 limiter. The new code never
-- reads or writes it; kept here for one deploy cycle in case of rollback.
-- Drop in a follow-up migration once the new limiter is verified.
